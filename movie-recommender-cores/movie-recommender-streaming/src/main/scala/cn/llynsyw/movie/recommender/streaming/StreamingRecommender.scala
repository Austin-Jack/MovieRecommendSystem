...
import cn.llynsyw.movie.recommender.commons.model.{MongoConfig, MovieRecs}
...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    new Thread(new Runnable {
      override def run(): Unit = new LogProcessor().startProcess()
    }).start()
      ...
      rdds =>
        rdds.foreach {
          case (uid, mid, score, timestamp) => {
            println($"rating data coming! >>>>>>>>>>>>>>>> uid:${uid},mid:${mid},score:${score},timestamp:${timestamp}")
  ...
  def computeMovieScores(candidateMovies: Array[Int],
    ...
    val increMap: mutable.Map[Int, Int] = scala.collection.mutable.HashMap[Int, Int]()
    val decreMap: mutable.Map[Int, Int] = scala.collection.mutable.HashMap[Int, Int]()
